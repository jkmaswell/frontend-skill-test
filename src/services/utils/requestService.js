...
const baseApi = 'https://frontend-skill-test.herokuapp.com/'
...
class RequestService {
  ...
  get (url, options = {}) {
    ...
    return this.axios.get(`${baseApi}${url}`, config)
      .then(this.getTransformFunction(options.transformFunction))
  }

  post (url, body = {}, options = {}) {
    const { config = {} } = options
    return this.axios.post(`${baseApi}${url}`, body, config)
  }

  put (url, body = {}, options = {}) {
    const { config = {} } = options
    return this.axios.put(`${baseApi}${url}`, body, config)
      .then(this.getTransformFunction(options.transformFunction))
      .catch(err => err)
  }
  ...
  patch (url, body = {}, options = {}) {
    const { config = {} } = options
    return this.axios.patch(`${baseApi}${url}`, body, config)
      .then(this.getTransformFunction(options.transformFunction))
      .catch(err => err)
  }
  ...
  delete (url, options = {}) {
    const { config = {} } = options
    return this.axios.delete(`${baseApi}${url}`, config)
      .then(this.getTransformFunction(options.transformFunction))
  }
